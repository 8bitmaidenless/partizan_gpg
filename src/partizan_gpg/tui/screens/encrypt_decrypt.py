...
_MODES: list[tuple[str, str, bool, bool]] = [
    ...
    ("7. Verify detached", "7", False, True),
]

_MODE_LABELS = [m[0] for m in _MODES]
_MODE_NEEDS_RECIP = [m[2] for m in _MODES]
_MODE_NEEDS_SIG_AREA = [m[3] for m in _MODES]
...
#ed-op-log {
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="ed-main"):
            with Vertical(id="ed-left"):
                ...
                yield TextArea("", id="ed-input-text", show_line_numbers=False)
                yield Input(
                ...
                yield TextArea("", id="ed-sig-text", show_line_numbers=False)
                ...
                yield Static(
                    "  Signing key: move cursor to it (no [b]Space[/b] needed) →",
                    id="ed-sign-hint"
                )
                ...
                yield TextArea("", id="ed-output", show_line_numbers=False)
    ...
    def action_set_mode(self, idx_str: str) -> None:
        idx = int(idx_str)
        try:
            buttons = list(self.query_one("#ed-mode-set", RadioSet).query(RadioButton))
    ...
    def _apply_mode(self, idx: int) -> None:
        self._mode_idx = idx
        needs_sig_area = _MODE_NEEDS_SIG_AREA[idx]
        ...
        sign_hint.display = (idx == 0 and bool(sign_cb.value))

        sig_label.display = needs_sig_area
        sig_text.display = needs_sig_area

        if idx == 0:
            key_label.update(
                "[u]Recipients[/u]  "
                "[i]([b]Space[/b] = select, [b]cursor[/b] = signing key if ticked)[/i]"
            )
        elif idx in (3, 5):
            key_label.update(
                "[u]Signing key[/u]  "
                "[i](move [b]cursor[/b] to your key - no Space needed)[/i]"
            )
        else:
            key_label.update("[u]Keys[/u]  [i](reference only)[/i]")
    ...
    def _apply_input_mode(self, mode: str) -> None:
        ...
        self.query_one("#ed-input-text", TextArea).display = (mode == "text")
        self.query_one("#ed-input-file", Input).display = (mode == "file")
        self.query_one("#ed-btn-text", Button).variant = "primary" if mode == "text" else "default"
        self.query_one("#ed-btn-file", Button).variant = "primary" if mode == "file" else "default"

    def on_checkbox_changed(self, event: Checkbox.Changed) -> None:
        if event.checkbox.id == "ed-opt-sign":
            self.query_one("#ed-sign-hint", Static).display = (
                bool(event.value) and self._mode_idx == 0
            )
    ...
    def on_key_list_widget_selection_changed(
        ...
        if n == 0:
            ...
            status.update(
                f"  ⚠ Signing uses the [b]cursor[/b], not [b]Space[/b]. "
                f"Clear selection with [b]Esc[/b]."
            )
        else:
    ...
    def _get_input_str(self) -> str | None:
        ...
        try:
            ...
            self._log.log("Input is not valid UTF-8 text.", level="ERR")
            return None
        
    def _get_selected_fingerprints(self) -> list[str]:
        return list(self._key_list.get_selected())
    
    def _get_cursor_key(self) -> KeyInfo | None:
    ...
    def _run_encrypt_recipients(self) -> None:
        ...
        if not recipients:
            self._log.log(
                "Select at least one recipient key with [Space].",
                level="WARN"
            )
            return
        
        armor = self._armor_requested()
        will_sign = bool(self.query_one("#ed-opt-sign", Checkbox).value)

        if will_sign:
            signer_info = self._get_cursor_key()
            if signer_info is None:
                self._log.log(
                    "Move the cursor to your signing key (arrow keys). "
                    "It does not need to be Space-selected.",
                    level="WARN"
                )
                return
            
            if signer_info.fingerprint in recipients:
                self._log.log(
                    f"⚠ Signing key ({signer_info.key_id}) is also a recipient - "
            ...
            self.app.push_screen(
                PassphraseModal(
                    title=f"Passphrase for signing key: {signer_info.name} "
                          f"<{signer_info.key_id}>"
                ),
    ...
    def _on_encrypt_sign_pass(self, result: PassphraseResult) -> None:
        ...
        if result.cancelled:
            self._log.log("Encrypt+sign cancelled.", level="INFO")
            return
        if result.was_empty:
            self._log.log(
    ...
    def _finish_encrypt_recipients(
        ...
        self._log.log_result(
            ...
            detail=(
                f"{len(result)} chars" if ok and isinstance(result, str)
                else f"{len(result)} bytes" if ok
                else None
            )
        )
    ...
    def _run_encrypt_symmetric(self) -> None:
        ...
        if plaintext is None:
            return
        self._pending = {"plaintext": plaintext}
        self.app.push_screen(
    ...
    def _on_sym_passphrase(self, result: PassphraseResult) -> None:
        ...
        if result.was_empty:
            self._log.log(
                "Empty passphrase - symmetric ciphertext will use a blank key.",
                level="WARN"
            )
    ...
    def _finish_encrypt_symmetric(self, result: str | bytes | None) -> None:
        self._log.log_result(ok=result is not None, label="encrypt_symmetric")
        self._write_output(result)
    ...
    def _run_decrypt(self) -> None:
        ...
        if ciphertext is None:
            return
        self._pending = {"ciphertext": ciphertext}
        self.app.push_screen(
            PassphraseModal(
                title="Passphrase (leave blank for asymmetric / agent-cached key)"
            ),
    ...
    def _on_decrypt_passphrase(self, result: PassphraseResult) -> None:
        ...
        self.run_worker(
            self._worker_decrypt(pending["ciphertext"], passphrase),
            thread=True,
            name="decrypt"
        )
    ...
    def _finish_decrypt(self, result: bytes | None) -> None:
        self._log.log_result(
            ok=result is not None,
            label="decrypt_data",
            detail=f"{len(result)} bytes plaintext" if result is not None else None
        )
        self._write_output(result)
    ...
    def _run_clearsign(self) -> None:
        ...
        if signer_info is None:
            ...
            return
        if not signer_info.has_secret:
            self._log.log(
                f"No secret key available for {signer_info.name} "
                f"({signer_info.key_id}) - import or generate a secret key first.",
                level="ERR"
            )
            return
        ...
        self.app.push_screen(
            ...
            callback=self._on_clearsign_pass
        )

    def _on_clearsign_pass(self, result: PassphraseResult) -> None:
        pending = self._pending
        self._pending = {}

        if result.cancelled:
            self._log.log("Clearsign cancelled.", level="INFO")
            return
        if result.was_empty:
            self._log.log(
                "Empty passphrase - assuming key has no passphrase.",
                level="WARN"
            )

        self.run_worker(
            self._worker_clearsign(
                pending["message"],
                pending["sign_fp"],
                result.passphrase
            ),
    ...
    async def _worker_verify_clearsign(self, signed_msg: str) -> None:
        vr = await asyncio.to_thread(verify_clearsign, self.gpg, signed_msg)
    ...
    def _run_sign_detached(self) -> None:
        ...
        if not signer_info.has_secret:
            self._log.log(
                f"No secret key for {signer_info.name} ({signer_info.key_id}).",
                level="ERR"
            )
            return
        ...
        self.app.push_screen(
            ...
            callback=self._on_detached_sign_pass
        )

    def _on_detached_sign_pass(self, result: PassphraseResult) -> None:
        pending = self._pending
        self._pending = {}

        if result.cancelled:
            self._log.log("Detached sign cancelled.", level="INFO")
            return
        if result.was_empty:
            self._log.log(
                "Empty passphrase - assuming key has no passphrase.",
    ...
    def _run_verify_detached(self) -> None:
        ...
        sig_text = self.query_one("#ed-sig-text", TextArea).text.strip()
        if not sig_text:
            self._log.log(
                "Paste the detached .asc signature into the signature field.",
                level="WARN"
            )
            return
...
#             }
#             self.app.push_screen(
#                 PassphraseModal(title=f"Passphrase for signing key: <{sign_fp[-16:]}>"),
#                 callback=self._on_encrypt_pass
#             )
#         else:
#             self.run_worker(
#                 self._worker_encrypt_recipients(
...
#     def _on_encrypt_pass(self, result: PassphraseResult):
#         _pending = self._pending_dict
#         self._pending_dict = None
#         if result.cancelled:
#             self._log.log(
#                 "Encrypt w/ sign cancelled",
#                 level="INFO"
#             )
#             return
#         if result.was_empty:
...
#     def _on_clearsign_pass(self, result: PassphraseResult) -> None:
#         sign_fp = getattr(self, "_sign_fp", None)
...
#             )
#         self.run_worker(
#             self._worker_clearsign(message, sign_fp, result.passphrase),
#             thread=True,
#             name="clearsign"
#         )

#     async def _worker_clearsign(self, message: str, sign_fp: str, passphrase: str | None) -> None:
#         result = await asyncio.to_thread(
#             clearsign,
#             self.gpg,
#             message,
...
#     async def _worker_verify_clearsign(self, signed_msg: str) -> None:
#         from partizan_gpg.cipherlib import verify_clearsign as _verify_cs
...
#         self.app.push_screen(
#             PassphraseModal(title=f"Passphrase for signing key <{sign_fp[-16:]}>"),
#             callback=self._on_detached_pass
#         )

#     def _on_detached_pass(self, result: PassphraseResult) -> None:
#         sign_fp = getattr(self, "_sign_fp", None)
#         message = getattr(self, "_pending_message", None)
#         if sign_fp is None:
#             return
...
#             )
#         self.run_worker(
#             self._worker_sign_detached(message, sign_fp, result.passphrase),
#             thread=True,
#             name="sign_detached"
#         )
