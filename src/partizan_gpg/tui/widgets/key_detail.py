...
from partizan_gpg.tui.widgets.key_list import (
    KeyInfo,
    _expand_trust,
    _format_algorithm,
    _format_expiry
...
.kd-subkey-table > .datatable--header {
    background: $primary-darken-2;
    color: $text-muted;
    text-style: bold;
}

.kd-subkey-table > .datatable--even-row {
    background: $surface;
}

.kd-subkey-table > .datatable--odd-row {
    background: $background;
}
...
if __name__ == "__main__":
    ...
    try:
        from partizan_gpg.cipherlib import build_gpg, generate_key
    except ImportError:
